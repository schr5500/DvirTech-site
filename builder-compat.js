...
const DVT_RULES = [
  ...
  {
    /* 🔴 **אורך כבל ה-CPU (‏EPS) מול גובה המארז — תצפית שטח של דביר
       (27.08): "אורך כבל ה-CPU שמגיע מהספק תמיד מגיע על הכסכס...
       במידה ולקוח קונה FULL-TOWER — זה לא נראה לי יגיע."**

       והיא נכונה, ואומתה מול מקורות חיצוניים לפני שנכתב הכלל:
       ‏EPS סטנדרטי הוא 60-65 ס"מ (‏AZZA מפרסמת 600 מ"מ במפרט,
       ‏Corsair RM — ‏650 מ"מ), המסלול בפועל הוא מהספק בתחתית, מאחורי
       מגש הלוח, עד הפינה השמאלית-עליונה — וב-Full Tower ההמלצה
       המקובלת היא ~700 מ"מ. פורומים מלאים ב"חסר לי 2 אינץ'".
       הפתרון בשטח: מאריך EPS (‏20-30 ס"מ, זול) — אבל צריך לדעת
       **לפני** ההרכבה, לא באמצע.

       ⚠️ warn ולא error: זה פתיר (מאריך/ניתוב קדמי), והנתונים חלקיים.
       ⚠️ שלוש רמות ידיעה:
         • towerType="Full" (או שם המוצר מסגיר) + אורך ידוע < 70 —
           אזהרה עם המספרים.
         • ‏Full + אורך לא ידוע — אזהרה כללית: הסטנדרט לא מגיע.
         • לא-Full + אורך ידוע ≤ 55 — הערת info: צמוד גם ב-Mid.
       ⚠️ זיהוי Full משם המוצר הוא נסיגה בלבד — עמודת towerType
       בגיליון גוברת כשמולאה. */
    id:"psu-eps-cable", cats:["psu","case"], on:"psu", level:"warn",
    wants:["psu.epsCableCm","case.towerType"],
    run:({psu,cs}) => {
      const eps = dvtNumOf(psu,"epsCableCm");
      const tower = String(dvtGet(cs,"towerType") || "").trim().toLowerCase();
      const looksFull = tower === "full" ||
        (!tower && /full[\s-]*tower|פול\s*טאוו?אר/i.test(String(cs.name || "")));

      if(looksFull){
        if(eps && eps >= 70) return null;
        if(eps){
          return `כבל ה-CPU של הספק ${psu.name} הוא ${eps} ס"מ, ובמארז Full Tower ` +
                 `הניתוב מאחורי הלוח דורש ~70 ס"מ. סביר שיידרש מאריך EPS — נוודא לפני ההרכבה.`;
        }
        return `במארז Full Tower כבל ה-CPU של ספקים סטנדרטיים (60-65 ס"מ) מגיע בקושי ` +
               `או לא מגיע בניתוב מאחורי הלוח. נבדוק את אורך הכבל של הספק לפני ההרכבה, ` +
               `ובמידת הצורך נוסיף מאריך EPS.`;
      }
      return null;
    }
  },
  {
    /* בן-הזוג העדין: כבל שידוע כקצר מהסטנדרט צמוד גם במארז רגיל. */
    id:"psu-eps-cable-short", cats:["psu"], on:"psu", level:"info",
    needs:["psu.epsCableCm"],
    run:({psu}) => {
      const eps = dvtNumOf(psu,"epsCableCm");
      return (eps && eps <= 55)
        ? `כבל ה-CPU של הספק ${psu.name} קצר מהסטנדרט (${eps} ס"מ) — בניתוב מאחורי הלוח הוא יגיע צמוד. נתחשב בזה בהרכבה.`
        : null;
    }
  },
  {
