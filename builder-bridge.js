...
(() => {
  ...
  /* 🔴 **מארז לדיסקים אינו מארז מחשב.** הדפוס הקודם חיפש רק
     "מארז חיצוני", ולכן CSE-7003 — "מארז ל4 דיסקים תומך RAID" —
     עבר לבונה. לקוח יכול היה לבחור אותו כשלדה למחשב שלם, וכל כללי
     התאימות של המארז מדלגים בשקט (כל השדות שלהם ריקים) — כלומר
     ההרכבה מדווחת **תקינה**.

     ⚠️ הסינון לפי **שם** ולא לפי שדות ריקים, בכוונה: CSE-7053
     (Corsair 3000D) הוא מארז PC אמיתי שחסר לו `supportedFormFactors`,
     וסינון לפי שדות היה מוחק אותו מהבונה בטעות.
     ⚠️ `מארז\s*(ל[-\s]?)?\d` תופס "מארז ל4 דיסקים" ו"מארז 2 דיסקים"
     בלי לפגוע ב"מארז Corsair 3000D" — שם המספר בא אחרי שם המותג. */
  function isBuildableCaseItem(i){
    const n = String((i && i.name) || "");
    if(/מארז\s*חיצוני|enclosure|docking|bay\b/i.test(n)) return false;
    if(/מארז\s*(ל[-\s]?)?\d+\s*דיסק/i.test(n)) return false;   // "מארז ל4 דיסקים"
    if(/דיסקים.*RAID|RAID.*דיסקים/i.test(n)) return false;
    return true;
  }
