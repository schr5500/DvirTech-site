/* =====================================================================
   ...
   - כרטיס רגיל מול תשלום מזומן/Bit: אי אפשר להראות "עמלת אשראי" בנפרד
     ...
     אחד או שניים לא מציגים/מוסיפים שום עמלה — המחיר תמיד זהה.**
     (עודכן 25.08: מסלול 3 תשלומים עבר לעמלה מפורשת, 0.9% לתשלום.)
...
/* 🔴 **עודכן 25.08 בהוראת דביר — "אנחנו לא משלמים על זה יותר":** גם
   מסלול 3 תשלומים נושא עמלה מפורשת, לפי תעריף הניכיון של UPAY לעסקה
   עד 3 תשלומים (0.9% לתשלום). חינם: 1-2 תשלומים בלבד.
   ⚠️ אותם שלושה קבועים בדיוק ב-4-payment-api.gs — השרת קובע. */
const INSTALLMENT_FEE_PCT_1TO3_PRE_VAT = 0.9;
const INSTALLMENT_FEE_FREE_UPTO = 2;                 // 1–2 תשלומים: ללא עמלה
const VAT_RATE = 0.18;
...
/*
   ...
   🔴 **עודכן 25.08:** גם מסלול 3 תשלומים נושא עמלה (0.9% × 3 + מע"מ,
   תעריף UPAY לעסקה עד 3). חינם: 1-2 תשלומים בלבד.
...
/* ==================== תוויות אפשרויות התשלומים ====================
   ...
   **אחרי** הבחירה. לקוח שכבר החליט "ארבעה תשלומים" וגילה 177 ₪
   ...
   עכשיו כל שורה נושאת את המחיר שלה, ו"עד 2 — ללא עמלה" יושב מול
   "3 — ‎+X ₪" באותה רשימה. הלקוח בוחר בעיניים פקוחות.
